...
import { getChild, trainingCoursewareDetail, edittrainingCourse, getteachers } from "@/api/training";
export default {
  data() {
    return {
  ...
  mounted() {
    ...
    trainingCoursewareDetail(this.id).then(res => {
      this.formData = res.data
      this.formData.imgUrl = res.data.course_thumb
      this.formData.video = res.data.link_url
      this.$refs.childImage.uploaderInfos.imageUrl = res.data.course_thumb
      this.$refs.childVideo.uploaderInfos.videoUrl = res.data.link_url
      this.$refs.childImage.uploaderInfos.progress = 1
      this.$refs.childVideo.uploaderInfos.progress = 1
    })
  },
}
</script>
